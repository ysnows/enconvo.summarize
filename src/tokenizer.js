...
export const getTokenLenght = async (text) => {
...
// export __ROOT_DIR__='/Users/ysnows/Library/Application Support/Enconvo'
async function main() {
    const resp = await getTokenLenght(`
首发于
【关于NLP】那些你不知道的事
...
​关注他
405 人赞同了该文章
近期LangChain + LLM 方案高速发展，降低了知识问答等下游应用的开发门槛。但随着业务深入，一些局限性也日渐显露，比如：LLM意图识别准确性较低，交互链路长导致时间开销大；Embedding 不适合多词条聚合匹配等。本文结合实际案例，分析上述问题存在的根源，并提出基于传统 NLP 技术的解决思路。
背景
...
1. 本地知识建立索引时，通常对单个知识点进行 Embedding；不会也不可能，为不同知识点的排列组合分别制作索引。不难想象，不同记录和属性的组合方式有 
 (C问题数，如过滤，查询，最值；A 即排列)，所有组合都建立索引的开销是巨大的。
...
有了 Agents 和 Chains的标准抽象，下面再来看看摸清用户意图的几种方法，这样才能开发合适的工具：
 ...
 。Precise Zero-Shot Dense Retrieval without Relevance Labels 一文面向 zero-shot场景下的稠密检索 ，使用基础模型在训练过程中已经掌握的相关语料，面向用户问题，生成虚构的文档。该文档的作用，不是输出最终结果，而是通过 LLM 对问题的理解能力，生成与之相关的内容。这相当于自动化生成相关性标签，避免外部输入。虚构文档生成后，再使用无监督检索器进行 Embedding；然后将生成的向量在本地知识库中进行相似性检索，寻找最终结果。上述过程在原论文中也提供了示意图：
...
李朋
李朋
感觉像是在做搜索系统，最后优化成关键词召回了，就没必要用向量引擎了
...
bubblebee
bubblebee
有这种感觉，就是变着方法做query expansion或document expansion。如果使用LLM的话有doc2query，包括现在sparse retrieval的splade。感觉都是把关键字范围扩展到整个词表上去更准确的匹配用户意图。不过据说从query expansion出发效果差些，可能query太短，信息量少些。
...
灰墙
灰墙
为啥不多路召回，搜索场景非常常用的做法，向量召回效果不一定好，传统方法不能完全丢弃[大笑]
...
小虫飞飞
作者
你是看到 n 个记录 m 个属性场景，很契合关系型数据库吗？这只是我给的一个抽象描述。如果数据源很规整，比如就是 csv文件，那用 sql 没问题，直接让 llm 生成查询语句即可；但真实场景，数据源格式多变复杂，恰恰需要先把关键词识别出来（比如文中的方法），把公共的列提取出来，这样才能进行后面的检索和对比。
07-04 · IP 属地上海
​回复
​3
邱震宇
邱震宇
好工作！关键词抽取那块，我之前也是类似做法，用的句法分析，总结适配场景的词语关系组合。llm的应用还是得结合当前一些比较好用的传统方法。[赞同]
07-05 · IP 属地江苏
​回复
​2
Alwin阿布
Alwin阿布
在前置任务都能意图识别了，再用LLM组织语言看似可行。细想怎么都觉的是鸡生蛋还是蛋生鸡的问题哦。我们很多时候就是因为做不好意图识别，才会想到用LLM去解决
...
小虫飞飞
作者
你说得对，并不冲突，结合场景吧，选择更合适的。
07-05 · IP 属地上海
​回复
​赞
pengsir
pengsir
请问下，把问题和知识集提取关键词后，是分别embedding后存入向量数据库做向量搜索，还是直接把关键词存入搜索引擎做关键词检索？
...
小虫飞飞
作者
都是可以的，embedding 有其他好处，可以过滤些噪音，另外恐怕也得结合自己的场景，比如向量有没有它用？其它服务是用普通文本匹配还是向量匹配的，可以综合考虑。
07-05 · IP 属地上海
​回复
​赞
木头哇
木头哇
楼主这个做法，有没有在比较多的例子上用过，感觉检索阶段已经做好了精确搜索了，按照文中的这种例子，检索出来的答案直接展示就好，感觉不必在过一遍LLM
...
小虫飞飞
作者
有时候需要 llm 整合，举个例子，前端支持 markdown 渲染，用户问题可能包含: 请用表格形式展示王超马汉的武力差异。
07-04 · IP 属地上海
​回复
​4
木头哇
木头哇
小虫飞飞
嗯嗯，的确。楼主这个好分享，LLM + 向量数据库的做法，向量召回精度问题的工作网上一直搜不到。很奇怪大家都在说结合向量引擎，实际应用没碰到这个问题吗
...
叶叶叶
叶叶叶
请问能够详细说一下这里面的多级索引怎么建立么？是说一个文档块得到多个关键短语，每个关键短语构建一个向量？
...
作者
幽王
分片的原始文档不必向量化；对关键词Embedding。关键词和原文本，放到vector store 中。存储方式和 vs 选型有关。大概的形式可能是：key，作为关键词 embedding 的结果；value，包含原文本、分片 id等杂七杂八的metadata。
07-06 · IP 属地上海
​回复
​2
小虫飞飞
...
皮蓬数据英格利什数据布兰德数据
提取关键词后 embedding，相比于整句embedding，相似度会更高。差异点就如文章开始的 “局限性分析” 描述所示：

整句 embedding 想要命中三条记录，需要增加 top k，降低相似度阈值。结果就是引入更多噪音。关键词列表分开 embedding 检索，相似度更高。所以 top k 可以较小，相似度阈值可以提高，避免引入过多无关文档。从而降低和 LLM 交互开销（时间、token 数量），也提升 LLM 的汇总分片的精度。
07-03 · IP 属地上海
​回复
​1
查看全部 6 条回复​
乔巴
乔巴
你好，有个问题哈，比如上面讲的得到了关键词列表[:'张指导', '姚主席']
['詹姆斯三分能力', '詹姆斯抢断能力','约基奇三分能力','约基奇抢断能力', '张伯伦三分能力', '张伯伦抢断能力']，后续如何去embedding，是分别对这个7个短语进行embedding去召回吗？
...
yuhui
yuhui
这玩意儿现在已经有公司在商用. 不过, 个人看法, 这条路可能还不是LLM 商业价值最大化的最佳范式, 或者说还里的比较远.
...
满天星alone
满天星alone
感谢分享，让我们有了新思路，不过同时也产生了两个疑问，还望大佬解答：
1、通过提取关键词的方式来表示一段内容，是会丢失一些语义的，原本两个比较相近的描述提取出来的关键词是不是就会更加趋同。
2、通过关键字embedding提升了匹配的精准度，那多个段文本之间的关联如何解决。
07-07 · IP 属地美国
​回复
​赞
小虫飞飞
...
作者
illusions
[赞]，你说的是对的，把 section + paragraph 都扔给 LLM 是会增加不少冗余信息，所以我说是常见方案，它不是很好的解决之道。我们这边有些优化，不过和场景结合较多。。。
...
illusions
小虫飞飞
我个人感觉哈，问题2你示例的解决方法很可能会引入大量冗余内容，或是急剧增加LLM负担。前者会进一步增加LLM的痼疾模型幻觉，后者对于GPT来说更费💰了，对于本地模型而言更可能爆内存了，感觉不是很好的解决方案，具体还得根据数据集来做精细的策略吧，这也是召回➕LLM问答两阶段之于微调好的单阶段LLM的劣势所在。或者作者大大有什么其他思路吗？
07-08 · IP 属地重庆
​回复
​赞
展开其他 1 条回复​
...
LLM有时候会编造资料中没有的内容，所以我现在觉得还不如直接输出搜索结果[捂脸]
07-07 · IP 属地北京
​回复
​赞
小虫飞飞
小虫飞飞
作者
可以的，结合实际场景测试下，综合考虑：要么改进 Prompt Template；要么改进语料；要么改进 LLM。
...
诶，现阶段LLM炼丹太依赖底层模型，没有稳可用的prompt，要是换底座，所有的prompt都必须重新校准。这个问题真是没啥好的办法啊~[发呆][发呆]
...
世界树的影子
世界树的影子
不如先用LLM生成知识图谱，查询的时候直接从知识图谱里面获取信息。向量数据库真是鸡肋。
...
johns wu
johns wu
之前做这块的时候尝试将中文wiki的词条（实体名）做embedding搜索来支持这块，思路都对上了，的确将段落（实体简述）做embedding会导致较难命中想要的目标，不过段落做embedding可以用来一些较为模糊且不太追求完全准确的场景，比如模仿特定的语气
...
小虫飞飞
作者
握爪。你对细分和整体 embedding 的优劣描述，和我体验一致。
...
小虫飞飞
作者
Redis，不过 pgvector 等也是可以的。真要工程化，不能使用内嵌内存 vs
...
郦遇
郦遇
赞，说到的问题也是我在用现在开源的问答系统时遇到的，基本思路和我现在硕士论文做的工作中一part差不多，但也就意味着我的论文的创新点又要再找了[大哭][大哭]
...
illusions
illusions
我觉得作者的思路很有启发性，但他的数据比较特殊，换一种数据很可能有不同的特性，那就可能有不同的“向量召回加强trick”~
07-08 · IP 属地重庆
​回复
​赞
郦遇
郦遇
小虫飞飞
我还在考虑仍然用LLM做提取的优化方法，总感觉LLM已经在手，再用传统方法不太优雅
...
小虫飞飞
作者
多轮对话，我这边主要是几个点: in context learning，语义槽引导用户填充，基于 chat history 的上下文聊天。其它可能有更多拓展，但线上没用过。
07-05 · IP 属地上海
​回复
​3
点击查看全部评论
...
推荐阅读
谈谈LLM文书的开头
常言道，万事开头难。有研究表明，33%的面试官会在面试最初的90秒内决定是否录用你。那么对于PS来说，开头的第一段则可以让招生官在近千份的PS中识别出你的与众不同。对于LLM的文书，如何言…

珀尔修斯
选择决定未来——LLM选课之我见┃总法律顾问手记78#
选择决定未来——LLM选课之我见┃总法律顾问手记78#
lily
发表于总法律顾问...
LLM入门级介绍：当我们谈论LLM的时候我们在谈论什么
新的申请季即将开始，相信有不少同学正在是否要出国、出国又应该选择什么专业就读的选择中挣扎。我们以往的公众号发过大量关于美国法学院申请的干货，为希望出国就读JD或LLM的学生提供了关…

蒋小诺
发表于律政留学
【留学观点】BU学姐带你看LLM和JD的区别
【留学观点】BU学姐带你看LLM和JD的区别
AdmitWrite留学平台
